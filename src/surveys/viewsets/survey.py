...
class SurveyViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in ['list','create','update','partial_update','delete','destroy','addQuestionToSurvey','deleteQuestionsFromSurvey','deleteDataCollectorsFromSurvey','uploadFilesForSurvey']:
            permission_classes = [custom_permissions.IsAdmin]
        elif self.action in ['retrieve']:
            permission_classes = [custom_permissions.IsAdmin | custom_permissions.IsDataCollector]
    ...
    def create(self, request , format = None):
        ...
        if not 'name' or not 'status' or not 'description' or not 'project_id' or not 'questions' in request.data.keys():
        ...
        for question in request.data['questions']:
            ...
            if len(question) == 0:
                return responses.BadRequestErrorHandler("Each Question must be non-empty")
            if not 'title' or not 'type' in question.keys():
                return responses.BadRequestErrorHandler("All required fields for a question must be input")
            

            if question['type'] not in ['CHOICE','OPEN','MEDIA']:
                return responses.BadRequestErrorHandler("question types must be CHOICE, MEDIA or OPEN")
            ...
            if question['type'] == 'CHOICE':
                ...
                if len(question['options']) == 0:
                    return responses.BadRequestErrorHandler("options is required for question type CHOICE")
                for option in question['options']:
                    if type(option) != str:
                        return responses.BadRequestErrorHandler("question options must be strings")
                questionOptions = question['options']
            
            if 'isDependent' in question.keys() and question['isDependent'] == True:
                if not isinstance(question['isDependent'],bool):
                    return responses.BadRequestErrorHandler('question isDependent must be boolean')
                if not 'depQuestion' in question.keys():
                    return responses.BadRequestErrorHandler('dependent question must have depQuestion')
                if not isinstance(question['depQuestion'],dict):
                    return responses.BadRequestErrorHandler("question depQuestion must be a dict")
                if not 'title' in question['depQuestion'].keys():
                    return responses.BadRequestErrorHandler("question depQuestion must have title")
                checkPreviousQuestion = Question.objects.filter(title = question['depQuestion']['title'],survey = createdSurvey)
                if not checkPreviousQuestion.exists():
                    return responses.BadRequestErrorHandler("The previous question was not found")

                if checkPreviousQuestion[0].type == 'CHOICE':
                    if not 'value' in question['depQuestion'].keys():
                        return responses.BadRequestErrorHandler("question depQuestion must have value for question type CHOICE")

                    if not checkPreviousQuestion.filter(options__contains=[question['depQuestion']['value']]).exists():
                        return responses.BadRequestErrorHandler("value in question depQuestion not in question choices")
                    ...
                    value = None
                    if question['depQuestion']['value'] and isinstance(question['depQuestion']['value'],str):
                        value = question['depQuestion']['value']
            ...
            if 'hasMultipleAnswers' in question.keys():
                if not isinstance(question['hasMultipleAnswers'],bool):
                    return responses.BadRequestErrorHandler('question hasMultipleAnswers must be boolean')
    ...
    def update(self, request, format = None, pk = None):
        ...
        for question in request.data['questions']:
            ...
            checkQuestion = Question.objects.filter(pk = question['id'],survey = checkSurvey[0])
    ...
    def addQuestionToSurvey(self, request , format = None,pk = None):
        ...
        for question in request.data['questions']:
            ...
            if 'isDependent' in question.keys() and question['isDependent'] == True:
                ...
                if not 'title' in question['depQuestion'].keys():
                    return responses.BadRequestErrorHandler("question depQuestion must have title")
                checkPreviousQuestion = Question.objects.filter(title = question['depQuestion']['title'],survey = checkSurvey[0])
                if not checkPreviousQuestion.exists():
                    return responses.BadRequestErrorHandler("The previous question was not found")
    ...
    def uploadFilesForSurvey(self, request, format = None , pk = None):
        ...
        checkAllQuestionForMedia = Question.objects.filter(survey = checkSurvey[0],type = 'MEDIA')

        # checks all question for survey with type media and no files
        checkAllQuestionForMediaWoutFiles = Question.objects.filter(survey = checkSurvey[0],type = 'MEDIA',audioURL = None,imageURL = None,videoURL = None)
        if not checkAllQuestionForMedia.exists():
            return responses.BadRequestErrorHandler("no question with type media exists")
        ...
        if len(request.data.keys()) < len(checkAllQuestionForMediaWoutFiles):
            return responses.BadRequestErrorHandler("responses must match the questions with type MEDIA")

        allowedAudioFiles = ['audio/ogg','audio/mp3','audio/wav','audio/m4a','audio/aac','audio/wma','audio/mpeg']
        allowedVideoFiles = ['video/mp4','video/mov','video/mkv','video/avi','video/gif','video/flv']
        allowedImageFiles = ['image/png','image/jpg','image/jpeg','image/bmp','image/svg','image/gif']
        ...
        for key in request.data.keys():
            ...
            if separator == -1:
                return responses.BadRequestErrorHandler("format of all keys must be id_${type}")
            ...
            if type not in ['audio','video','image']:
                return responses.BadRequestErrorHandler("all key types must be from audio, video and image")
            ...
            if not checkQuestion.exists():
                return responses.BadRequestErrorHandler("question does not exist for the survey")
    ...
    def addDataCollectorToSurvey(self, request, format = None, pk = None):
        ...
        if not request.data['dataCollectors']:
            return responses.BadRequestErrorHandler("dataCollectors array must not be empty")
        
        checkDataCollectors = ExtendedUser.objects.filter(pk__in = request.data['dataCollectors'], role__role = 'Data-Collector')
        
        if len(checkDataCollectors) != len(request.data['dataCollectors']):
            return responses.BadRequestErrorHandler("all data collectors were not found")

        checkExisting = checkSurvey[0].dataCollectors.all().filter(id__in = request.data['dataCollectors'])
        if checkExisting.exists():
            return responses.BadRequestErrorHandler("All data collectors to be added must not be assigned to the event")
    ...
    def deleteDataCollectorsFromSurvey(self, request , format = None, pk = None):
        ...
        if len(checkExisting) != len(checkDataCollectors):
            return responses.BadRequestErrorHandler("all data collectors were not assigned to the survey")
    ...
    def answerSurvey(self, request , format = None, pk = None):
        ...
        if len(request.data['answers']) < len(allQuestions.filter(isRequired = True)):
            return responses.BadRequestErrorHandler("all required questions must be answered")

        if len(request.data['answers']) > len(allQuestions.filter()):
            return responses.BadRequestErrorHandler("questions to be answered must not exceed the actual questions")

        for question in request.data['answers']:
            # checks all required fields are input
            if not 'question_id' or not 'response' in question:
                return responses.BadRequestErrorHandler("every question must have a question_id and response property")
            ...
            if not isinstance(question['response'],list):
                return responses.BadRequestErrorHandler("all response properties must be an array")
            ...
            checkQuestion = Question.objects.filter(pk = question['question_id'],survey = checkSurvey[0])
            if not checkQuestion.exists():
                return responses.BadRequestErrorHandler("question id for the survey does not exist")
            
            if len(question['response']) > 1 and checkQuestion[0].hasMultipleAnswers == False:
                return responses.BadRequestErrorHandler("question not allowed to have multiple answers")
            ...
            if checkQuestion[0].type == 'CHOICE':
                for response in question['response']:
                    if not checkQuestion.filter(options__contains=[response]).exists():
                        return responses.BadRequestErrorHandler("all answers for type choice must be in question options")

            # checks a dependent condition and the value 
            if checkQuestion[0].isDependent == True:
                checkDepQuestion = next((answer for answer in request.data['answers'] if answer['question_id'] == checkQuestion[0].depQuestion['id'] and answer['response'][0] == checkQuestion[0].depQuestion['value']),None)
                if checkDepQuestion == None:
                    return responses.BadRequestErrorHandler("dependepent question value not correct")
