...
def checkAdminToken(headers):
    if('Authorization') not in headers: return responses.BadRequestErrorHandler("Authorization token is required")
...
def checkAdminFromToken(token):
    ...
    if decodedToken['token_type'] != 'access': return False
    ...
    user = ExtendedUser.objects.filter(pk = decodedToken['user_id'],role__id = checkRole[0].pk)
    if user.exists():
        return True
    else:
        return False
