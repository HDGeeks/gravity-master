...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in ['list','create','retrieve','update','partial_update','delete','destroy','addDataCollector','getDataCollectors','getAdmins']:
    ...
    def addDataCollector(self, request , format = None):
        ...
        if not 'phone' or not 'email' or not 'first_name' or not 'last_name' or not 'username' in request.data.keys():
            return responses.BadRequestErrorHandler("All required fields must be input")

        # checks the user with the email or username already exists
        checkUser = ExtendedUser.objects.filter(Q(email = request.data['email']) | Q(username = request.data['username']))
        
        if checkUser.exists():
            return responses.BadRequestErrorHandler("User with the email or username already exists.")
        ...
        if not checkDataCollectorRole.exists():
            return responses.BadRequestErrorHandler("Data Collector role does not exist.")
    ...
    def getAdmins(self, request, format = None, pk = None):
        ...
        return responses.SuccessResponseHandler(
            True,
            "Successfully found admins",
            formatter.userFormatter(admins[1:])
        )