...
class AdminViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in ['list','create','retrieve','update','partial_update','delete','destroy','addAdmin']:
    ...
    def createAdmin(self, request , format = None):
        ...
        if not 'username' or not 'email' or not 'password' or not 'first_name' or not 'last_name' or not 'middle_name' or not 'phone'  in request.data.keys():
    ...
    def signin(self, request , format = None):
        ...
        user = ExtendedUser.objects.filter(email = request.data['email'],role = checkAdminRole[0])
